class FollowsController < ApplicationController
  ...
  def update
    follower = User.find(params[:user])
    follow = current_user.follows_inbound.pending.find_by(follower_id: follower.id)

    if follow.update(pending: false)
      redirect_to root_url, notice: "You have accepted #{follower.full_name}'s follow request"
    else
      redirect_to root_url, alert: "An error occured when trying to accept #{follower.full_name}'s follow request"
    end
  end
  ...
  def other_user_exists?
    ...
    redirect_to(root_path, alert: 'Other user does not exist!') unless other_user
  end
end
